...
def run_gae(args):
    ...
    data_size_mbyte = recons_data.nbytes / (1024 ** 2)
    for nrmse in all_nrmse:
        ...
        for alg in all_algs: #,  "Cascaded"
            ...
            encode_time = encoding_end - start_time
            decode_time = decoding_end - encoding_end
            overall_time = decoding_end - start_time

            encoding_speed = data_size_mbyte / max(encode_time, 1e-12)
            decoding_speed = data_size_mbyte / max(decode_time, 1e-12)
            speed_overall = data_size_mbyte / max(overall_time, 1e-12)

            print(
                alg,
                "Targe NRMSE: %.8f" % nrmse,
                "Final NRMSE: %.8f" % nrmse_final,
                "CR: %.3f" % CR,
                "Encode Time: %.3f s" % encode_time,
                "Decode Time: %.3f s" % decode_time,
                "Encode Speed: %.3f MB/s" % encoding_speed,
                "Decode Speed: %.3f MB/s" % decoding_speed,
                "Overall Speed: %.3f MB/s" % speed_overall,
            )
...
if __name__=="__main__":
    ...
    parser.add_argument('--path', type=str, default = "example.npz")
